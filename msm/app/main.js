app.controller("MainCtrl",function($scope,$http, $filter, $interval){
	...
	$scope.tcsServer = "http://192.168.100.50:3310/";
	...
	// $scope.portalHost = "http://localhost:57539/service/msm/";
	 ...
	 // $scope.serviceHost = host + ":28001/";
	 $scope.serviceHost = "http://localhost:60526/";
	 //$scope.serviceHost = "http://192.168.100.7" + ":28001/";
		...
		.ready(function(){
			...
			$('input.actid').numpad({
				onKeypadOpen: function(){
					$('.numHeader').html('ระบุรหัสกิจกรรม');
				},
				onKeypadClose: function(){
					$scope.issue.act = $(this).find('.nmpd-display').val();
